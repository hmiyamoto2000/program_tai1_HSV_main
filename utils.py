...
def calc_glcm_mean(img, mask, gray_level, d, mode=None):
    if(mode=="WHOLE"):
        ####fish->0, green back->255
        bgrLower = np.array([0, 255, 0])    # 抽出する色の下限(BGR)
        bgrUpper = np.array([0, 255, 0])    # 抽出する色の上限(BGR)
        img_mask = cv2.inRange(mask, bgrLower, bgrUpper) # BGRからマスクを作成
        #tifffile.imsave("./img.tif", img)
        #tifffile.imsave("./img_mask.tif", img_mask)
        ...
        ###0だけ別枠に分けるplan
        img_bin = img//(256//gray_level) # [0:255]->[0:7]
        #tifffile.imsave("./img_bin_before.tif", img_bin)
        img_bin[img_mask == 255] = 255
        #tifffile.imsave("./img_bin.tif", img_bin)
...
def calc_glcm_output(img, gray_level, d, mode=None):
    if(mode=="WHOLE"):
        ####fish->0, green back->255
        bgrLower = np.array([0, 255, 0])    # 抽出する色の下限(BGR)
        bgrUpper = np.array([0, 255, 0])    # 抽出する色の上限(BGR)
    ...
    #n_pairs_all = n_pairs_0 + n_pairs_45 + n_pairs_90 + n_pairs_135 + n_pairs_180 + n_pairs_225 + n_pairs_270 + n_pairs_315
    n_pairs_0_180 = n_pairs_0 + n_pairs_180
    #print(n_pairs_0, n_pairs_180)
    n_pairs_45_225 = n_pairs_45 + n_pairs_225
    #print(n_pairs_45, n_pairs_225)
    n_pairs_90_270 = n_pairs_90 + n_pairs_270
    #print(n_pairs_90, n_pairs_270)
    n_pairs_135_315 = n_pairs_135 + n_pairs_315
    #print(n_pairs_135, n_pairs_315)
    ...
    glcm_output = (glcmP_0_180 + glcmP_45_225 + glcmP_90_270 + glcmP_135_315) / 4.0 

    glcm_img = np.array(glcm_output*255., dtype="uint8")
    
    return glcm_img

def make_img_mask(img, mask ,gray_level):
...
def glszm_t(image, mask, BIN):
    ...
    # img = glszmFeatures._calculateMatrix
    # print(img)
    glszmFeatures.enableAllFeatures()
    #print(glszmFeatures.items())
    """
...
def calculationg(img_lbp, img_mask, point):
    ...
    # lbp_nor = lbp * 255 / 10
    # img_lbp = lbp_nor.astype(np.uint8)
    # tifffile.imsave("UniformLBP.tif", img_lbp)
...
def calculationg_img(img_lbp, img_mask, point):
    position = list(zip(*np.where((img_mask == 1))))
    lbp_list = []
